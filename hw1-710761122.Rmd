...
```{python ans22}
y=[list(dict_business.get('資本額').values())[i]>500000 for i in range(len(dict_business.get('資本額')))]
num_largeCapital=y.count(True)
...
```{python ans35}
key_title=[{'title':danceInfo[i]['title']} for i in range(numDance)]
key_showInfo=[{key:danceInfo[i]['showInfo'][0][key] for key in ['time','location']} for i in range(numDance) if danceInfo[i]['showInfo']!=[]]

danceInfoList = [{**key_title[i], **key_showInfo[i]} for i in range(numDance)]
danceInfoList
