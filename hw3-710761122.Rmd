...
library(reticulate)
use_python("C:/Users/kaiwun/ANACON~1/envs/myEnvironment",required = T)
...
df_jointP.set_axis(['sunny', 'cloudy', 'rainy'],axis=0,inplace=True)
df_jointP.set_axis(['happy', 'fair', 'sad', 'depressed'],axis=1,inplace=True)
...
```{python ans12}
marginP_weather = np.sum(joint_probability,axis=1)
#marginP_weather = [sum(df_jointP.iloc[i]) for i in range(0,3)]
marginP_mood = np.sum(joint_probability,axis=0)
#marginP_mood = [sum(df_jointP.iloc[:,i]) for i in range(0,4)]
...
```{python ans13}
df_jointP_cond_w=[df_jointP.iloc[i]/marginP_weather[i] for i in range(0,3)]
P_mood_cond_weather=pd.DataFrame(df_jointP_cond_w)

df_jointP_cond_m=[df_jointP.iloc[:,i]/marginP_mood[i] for i in range(0,4)]
P_weather_cond_mood=pd.DataFrame(df_jointP_cond_m)
...
```{python ans14}
p_ans14 = df_jointP.loc['sunny','sad']/sum(df_jointP.loc['sunny'])
q1ans['ans14']=p_ans14
```
...
```{python ans15}
a=dict(df_jointP.loc['cloudy'])
est_mood = list(a.keys())[list(a.values()).index(max(df_jointP.loc['cloudy']))]
...
```{python ans22}
def x_star(N, i, alpha=0.5):
    print(x_partition(N)[i-1]+alpha*(x_partition(N)[i]-x_partition(N)[i-1]))
...
rsum = sum(f(x_partition(N)[i-1]+alpha*(x_partition(N)[i]-x_partition(N)[i-1]))*(x_partition(N)[i]-x_partition(N)[i-1])for i in range(1,100))
...
```{python ans24}
def riemannsum(f,interval,N=101,alpha=0.5):
    x_partition=np.linspace(interval[0],interval[1],num=N)
    return sum(f(x_partition[i-1]+alpha*(x_partition[i]-x_partition[i-1]))*(x_partition[i]-x_partition[i-1])for i in range(1,N))
```
